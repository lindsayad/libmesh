...
#ifndef LIBMESH_UNSTRUCTURED_MESH_IMPL_H
#define LIBMESH_UNSTRUCTURED_MESH_IMPL_H
...
UnstructuredMeshTempl<RealType>::UnstructuredMeshTempl (const Parallel::Communicator & comm_in,
                                                        unsigned char d) :
  MeshBaseTempl<RealType> (comm_in,d)
{
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::copy_nodes_and_elements(const UnstructuredMesh & other_mesh,
                                               ...
                                               )
{
  ...
  const unsigned int n_old_node_ints = extra_int_maps.second.size(),
                     n_new_node_ints = this->_node_integer_names.size(),
                     n_old_elem_ints = extra_int_maps.first.size(),
                     n_new_elem_ints = this->_elem_integer_names.size();
  ...
  bool wrap_proc_ids = (this->_n_parts < other_mesh._n_parts);

  // We're assuming our subclass data needs no copy
  libmesh_assert_equal_to (this->_is_prepared, other_mesh._is_prepared);
    ...
    for (const auto & oldn : other_mesh.node_ptr_range())
      {
        processor_id_type added_pid = cast_int<processor_id_type>
          (wrap_proc_ids ? oldn->processor_id() % this->_n_parts : oldn->processor_id());
  ...
  //Copy in Elements
  {
    ...
    for (const auto & old : other_mesh.element_ptr_range())
      {
        ...
        if (old->has_children())
          for (unsigned int c = 0, nc = old->n_children(); c != nc; ++c)
            if (old->child_ptr(c) == RemoteElem::get_instance())
              el->add_child(const_cast<RemoteElem *>(RemoteElem::get_instance()), c);
        ...
        el->processor_id() = cast_int<processor_id_type>
          (wrap_proc_ids ? old->processor_id() % this->_n_parts : old->processor_id());
        ...
        if (!skip_find_neighbors)
          {
            for (auto s : old->side_index_range())
              if (old->neighbor_ptr(s) == RemoteElem::get_instance())
                el->set_neighbor(s, const_cast<RemoteElem *>(RemoteElem::get_instance()));
            this->add_elem(el);
          }
        else
          {
    ...
    if (skip_find_neighbors)
      {
        old_elems_to_new_elems[RemoteElem::get_instance()] = const_cast<RemoteElem*>(RemoteElem::get_instance());
...
template <typename RealType>
UnstructuredMeshTempl<RealType>::~UnstructuredMeshTempl ()
{
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::find_neighbors (const bool reset_remote_elements,
                                       const bool reset_current_list)
{
  ...
  if (reset_current_list)
    for (const auto & e : this->element_ptr_range())
      for (auto s : e->side_index_range())
        if (e->neighbor_ptr(s) != RemoteElem::get_instance() || reset_remote_elements)
        ...
        for (auto ms : element->side_index_range())
          ...
          next_side:
            ...
            if (element->neighbor_ptr(ms) == nullptr ||
                element->neighbor_ptr(ms) == RemoteElem::get_instance())
              {
   ...
   * parent.
   *
   * If a child element has a RemoteElem::get_instance() neighbor
  ...
  for (unsigned int level = 1; level < n_levels; ++level)
    {
      for (auto & current_elem : as_range(this->level_elements_begin(level),
                                          this->level_elements_end(level)))
        {
          ...
          for (auto s : current_elem->side_index_range())
            {
              if (current_elem->neighbor_ptr(s) == nullptr ||
                  (current_elem->neighbor_ptr(s) == RemoteElem::get_instance() &&
                   parent->is_child_on_side(my_child_num, s)))
                {
                        ...
                        (neigh->level()+1) == current_elem->level())))
                    {
                      ...
                      if (current_elem->active())
                        libmesh_assert_not_equal_to (current_elem->processor_id(),
                                                     this->processor_id());
#endif // DEBUG
                      neigh = const_cast<RemoteElem *>(RemoteElem::get_instance());
                    }
                  ...
                  // neighbor here becomes much harder.
                                     ...
                                     neigh->has_children()))
                    {
                      ...
                      while (neigh != RemoteElem::get_instance() &&
                             neigh->has_children())
                        {
                          ...
                          for (unsigned int c = 0, nc = neigh->n_children();
                               !found_neigh && c != nc; ++c)
                            {
                              ...
                              for (auto ncn : child->neighbor_ptr_range())
                                {
                                  if (ncn != RemoteElem::get_instance() &&
...
#ifdef DEBUG
                  if (neigh != nullptr && neigh != RemoteElem::get_instance())
          ...
          // children to search.
          if (pip == RemoteElem::get_instance() || pip->active())
            {
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::read (const std::string & name,
                             void *,
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::write (const std::string & name)
{
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::create_pid_mesh(UnstructuredMesh & pid_mesh,
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::create_submesh (UnstructuredMesh & new_mesh,
                                       ...
                                       const const_element_iterator & it_end) const
{
  ...
  const unsigned int n_node_ints = this->_node_integer_names.size(),
                     n_elem_ints = this->_elem_integer_names.size();
...
#ifdef LIBMESH_ENABLE_AMR
template <typename RealType>
bool UnstructuredMeshTempl<RealType>::contract ()
{
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::all_first_order ()
{
  /*
...
template <typename RealType>
void UnstructuredMeshTempl<RealType>::all_second_order (const bool full_ordered)
{
  // This function must be run on all processors at once
  parallel_object_only();
  ...
  for (; it != endit; ++it)
    {
      ...
      for (auto s : lo_elem->side_index_range())
        {
          if (lo_elem->neighbor_ptr(s) == RemoteElem::get_instance())
            so_elem->set_neighbor(s, const_cast<RemoteElem*>(RemoteElem::get_instance()));
        }
...
#endif // LIBMESH_UNSTRUCTURED_MESH_IMPL_H
