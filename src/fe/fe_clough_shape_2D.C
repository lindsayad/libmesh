...
static GeomReal d1d2n, d1d3n, d2d3n, d2d1n, d3d1n, d3d2n;
static GeomReal d1xd1x, d1xd1y, d1xd2n, d1xd3n;
static GeomReal d1yd1x, d1yd1y, d1yd2n, d1yd3n;
static GeomReal d2xd2x, d2xd2y, d2xd3n, d2xd1n;
static GeomReal d2yd2x, d2yd2y, d2yd3n, d2yd1n;
static GeomReal d3xd3x, d3xd3y, d3xd1n, d3xd2n;
static GeomReal d3yd3x, d3yd3y, d3yd1n, d3yd2n;
static GeomReal d1nd1n, d2nd2n, d3nd3n;
// Normal vector naming: N01x is the x component of the
// unit vector at point 0 normal to (possibly curved) side 01
static GeomReal N01x, N01y, N10x, N10y;
static GeomReal N02x, N02y, N20x, N20y;
static GeomReal N21x, N21y, N12x, N12y;
...
void clough_compute_coefs(const Elem * elem)
{
  ...
  N01x = dydxi[0];
  N01y = - dxdxi[0];
  Nlength = std::sqrt(static_cast<GeomReal>(N01x*N01x + N01y*N01y));
  N01x /= Nlength; N01y /= Nlength;

  N10x = dydxi[1];
  N10y = - dxdxi[1];
  Nlength = std::sqrt(static_cast<GeomReal>(N10x*N10x + N10y*N10y));
  N10x /= Nlength; N10y /= Nlength;

  N02x = - dydeta[0];
  N02y = dxdeta[0];
  Nlength = std::sqrt(static_cast<GeomReal>(N02x*N02x + N02y*N02y));
  N02x /= Nlength; N02y /= Nlength;

  N20x = - dydeta[2];
  N20y = dxdeta[2];
  Nlength = std::sqrt(static_cast<GeomReal>(N20x*N20x + N20y*N20y));
  N20x /= Nlength; N20y /= Nlength;

  N12x = dydeta[1] - dydxi[1];
  N12y = dxdxi[1] - dxdeta[1];
  Nlength = std::sqrt(static_cast<GeomReal>(N12x*N12x + N12y*N12y));
  N12x /= Nlength; N12y /= Nlength;

  N21x = dydeta[1] - dydxi[1];
  N21y = dxdxi[1] - dxdeta[1];
  Nlength = std::sqrt(static_cast<GeomReal>(N21x*N21x + N21y*N21y));
