...
#ifndef LIBMESH_MESH_BASE_IMPL_H
#define LIBMESH_MESH_BASE_IMPL_H
...
MeshBaseTempl<RealType>::MeshBaseTempl (const Parallel::Communicator & comm_in,
                                        unsigned char d) :
  MeshAbstract (comm_in),
...
template <typename RealType>
MeshBaseTempl<RealType>::MeshBaseTempl (const MeshBase & other_mesh) :
  MeshAbstract (other_mesh),
  boundary_info  (new BoundaryInfo(*this)),
...
template <typename RealType>
MeshBaseTempl<RealType>::MeshBaseTempl(MeshBase &&) = default;



template <typename RealType>
MeshBaseTempl<RealType>::~MeshBaseTempl()
{
  this->clear();
...
template <typename RealType>
void MeshBaseTempl<RealType>::set_elem_dimensions(const std::set<unsigned char> & elem_dims)
{
#ifdef DEBUG
...
template <typename RealType>
unsigned int MeshBaseTempl<RealType>::spatial_dimension () const
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::set_spatial_dimension(unsigned char d)
{
...
template <typename RealType>
unsigned int MeshBaseTempl<RealType>::add_elem_integer(const std::string & name,
                                        bool allocate_data)
{
...
template <typename RealType>
std::vector<unsigned int> MeshBaseTempl<RealType>::add_elem_integers(const std::vector<std::string> & names,
                                                      bool allocate_data)
{
...
template <typename RealType>
unsigned int MeshBaseTempl<RealType>::get_elem_integer_index(const std::string & name) const
{
  for (auto i : index_range(_elem_integer_names))
    if (_elem_integer_names[i] == name)
      return i;
...
template <typename RealType>
bool MeshBaseTempl<RealType>::has_elem_integer(const std::string & name) const
{
...
template <typename RealType>
unsigned int MeshBaseTempl<RealType>::add_node_integer(const std::string & name,
                                        bool allocate_data)
{
...
template <typename RealType>
std::vector<unsigned int> MeshBaseTempl<RealType>::add_node_integers(const std::vector<std::string> & names,
                                                      bool allocate_data)
{
  std::unordered_map<std::string, std::size_t> name_indices;
  for (auto i : index_range(_node_integer_names))
...
template <typename RealType>
unsigned int MeshBaseTempl<RealType>::get_node_integer_index(const std::string & name) const
{
  for (auto i : index_range(_node_integer_names))
    if (_node_integer_names[i] == name)
      return i;
...
template <typename RealType>
bool MeshBaseTempl<RealType>::has_node_integer(const std::string & name) const
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::prepare_for_use (const bool skip_renumber_nodes_and_elements, const bool skip_find_neighbors)
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::clear ()
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::subdomain_ids (std::set<subdomain_id_type> & ids) const
{
...
template <typename RealType>
subdomain_id_type MeshBaseTempl<RealType>::n_subdomains() const
{
  // This requires an inspection on every processor
  parallel_object_only();
...
template <typename RealType>
dof_id_type MeshBaseTempl<RealType>::n_nodes_on_proc (const processor_id_type proc_id) const
{
...
template <typename RealType>
dof_id_type MeshBaseTempl<RealType>::n_elem_on_proc (const processor_id_type proc_id) const
{
...
template <typename RealType>
dof_id_type MeshBaseTempl<RealType>::n_active_elem_on_proc (const processor_id_type proc_id) const
{
...
template <typename RealType>
dof_id_type MeshBaseTempl<RealType>::n_sub_elem () const
{
  dof_id_type ne=0;
...
template <typename RealType>
dof_id_type MeshBaseTempl<RealType>::n_active_sub_elem () const
{
  dof_id_type ne=0;
...
template <typename RealType>
std::string MeshBaseTempl<RealType>::get_info() const
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::print_info(std::ostream & os) const
{
  os << this->get_info()
     << std::endl;
}
...
template <typename RealType>
void MeshBaseTempl<RealType>::partition (const unsigned int n_parts)
{
...
template <typename RealType>
unsigned int MeshBaseTempl<RealType>::recalculate_n_partitions()
{
  // This requires an inspection on every processor
  parallel_object_only();
...
template <typename RealType>
const PointLocatorBase & MeshBaseTempl<RealType>::point_locator () const
{
  libmesh_deprecated();
...
template <typename RealType>
std::unique_ptr<PointLocatorBase> MeshBaseTempl<RealType>::sub_point_locator () const
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::clear_point_locator ()
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::set_count_lower_dim_elems_in_point_locator(bool count_lower_dim_elems)
{
...
template <typename RealType>
bool MeshBaseTempl<RealType>::get_count_lower_dim_elems_in_point_locator() const
{
...
template <typename RealType>
std::string & MeshBaseTempl<RealType>::subdomain_name(subdomain_id_type id)
{
  return _block_id_to_name[id];
}

template <typename RealType>
const std::string & MeshBaseTempl<RealType>::subdomain_name(subdomain_id_type id) const
{
...
template <typename RealType>
subdomain_id_type MeshBaseTempl<RealType>::get_id_by_name(const std::string & name) const
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::cache_elem_dims()
{
  // This requires an inspection on every processor
  parallel_object_only();
...
template <typename RealType>
void MeshBaseTempl<RealType>::detect_interior_parents()
{
  // This requires an inspection on every processor
  parallel_object_only();
...
template <typename RealType>
void MeshBaseTempl<RealType>::set_point_locator_close_to_point_tol(Real val)
{
...
template <typename RealType>
Real MeshBaseTempl<RealType>::get_point_locator_close_to_point_tol() const
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::size_elem_extra_integers()
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::size_node_extra_integers()
{
...
MeshBaseTempl<RealType>::merge_extra_integer_names(const MeshBase & other)
{
...
template <typename RealType>
void MeshBaseTempl<RealType>::remove_ghosting_functor(GhostingFunctorBase & ghosting_functor_base)
{
  auto & ghosting_functor = static_cast<GhostingFunctor &>(ghosting_functor_base);
...
template <typename RealType>
void MeshBaseTempl<RealType>::add_ghosting_functor(GhostingFunctorBase & ghosting_functor)
{
  _ghosting_functors.insert(&static_cast<GhostingFunctor &>(ghosting_functor));
}

template <typename RealType>
void MeshBaseTempl<RealType>::add_ghosting_functor(std::shared_ptr<GhostingFunctorBase> ghosting_functor)
{
  _shared_functors[std::dynamic_pointer_cast<GhostingFunctor>(ghosting_functor).get()]
    = std::dynamic_pointer_cast<GhostingFunctor>(ghosting_functor);
  this->add_ghosting_functor(*ghosting_functor);
}

} // namespace libMesh

#endif // LIBMESH_MESH_BASE_IMPL_H
