...
#endif
  const MeshBaseTempl<RealType> & _mesh;
...
private:
  ...
  void apply_dirichlet_impl(const ConstElemRangeTempl<RealType> & range,
...
public:
  ConstrainDirichlet (DofMap & dof_map_in,
                      const MeshBaseTempl<RealType> & mesh_in,
                      const Real time_in,
...
void DofMap::create_dof_constraints(const MeshBaseTempl<RealType> & mesh, Real time)
{
  ...
  ConstElemRangeTempl<RealType> range (mesh.local_elements_begin(),
                                       mesh.local_elements_end());

  // Global computation fails if we're using a FEMFunctionBase BC on a
  // ReplicatedMesh in parallel
  // ConstElemRangeTempl<RealType> range (mesh.elements_begin(),
  //                       mesh.elements_end());
  ...
  for (unsigned int variable_number=0; variable_number<this->n_variables();
    ...
    Threads::parallel_for (range,
                           ComputeConstraints<RealType> (_dof_constraints,
                                                         *this,
#ifdef LIBMESH_ENABLE_PERIODIC
                                               *_periodic_boundaries,
#endif
  ...
  for (DirichletBoundaries::iterator
       ...
       i != _dirichlet_boundaries->end(); ++i, range.reset())
    {
      ...
      Threads::parallel_for
        (range,
         ConstrainDirichlet<RealType>(*this, mesh, time, **i,
...
DofMap::build_sparsity (const MeshBaseTempl<RealType> & mesh) const
{
  ...
  Threads::parallel_reduce (ConstElemRangeTempl<RealType> (mesh.active_local_elements_begin(),
...
#define INSTANTIATE_DOF_MAP_REALTYPE_METHODS(RealType)                                             \
  ...
  template void DofMap::merge_ghost_functor_outputs(                                               \
      gf_map_type<RealType> &,                                                                     \
      std::set<CouplingMatrix *> &,                                                                \
      const typename std::set<GhostingFunctorBase *>::iterator & gf_begin,                         \
      const typename std::set<GhostingFunctorBase *>::iterator & gf_end,                           \
      const const_element_iterator_templ<RealType> & elems_begin,                                  \
      const const_element_iterator_templ<RealType> & elems_end,                                    \
      processor_id_type p)
