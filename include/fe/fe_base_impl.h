...
#ifndef LIBMESH_FE_BASE_IMPL_H
#define LIBMESH_FE_BASE_IMPL_H
...
struct Builder<Real, RealType>
{
  static std::unique_ptr<FEGenericBase<Real,RealType>> build(const unsigned int dim, const FEType & fet)
  {
    switch (dim)
    {
      // 0D
...
template <typename RealType>
struct Builder<RealVectorValue, RealType>
{
  static std::unique_ptr<FEGenericBase<RealVectorValue, RealType>> build(const unsigned int dim,
                                                                         const FEType & fet)
  {
    switch (dim)
    {
      ...
      case 0:
      {
        ...
        {
          case LAGRANGE_VEC:
            return libmesh_make_unique<FELagrangeVec<0,RealType>>(fet);

          case MONOMIAL_VEC:
            return libmesh_make_unique<FEMonomialVec<0,RealType>>(fet);
      ...
      case 1:
      {
        ...
        {
          case LAGRANGE_VEC:
            return libmesh_make_unique<FELagrangeVec<1,RealType>>(fet);

          case MONOMIAL_VEC:
            return libmesh_make_unique<FEMonomialVec<1,RealType>>(fet);
      ...
      case 2:
      {
        ...
        {
          case LAGRANGE_VEC:
            return libmesh_make_unique<FELagrangeVec<2,RealType>>(fet);

          case MONOMIAL_VEC:
            return libmesh_make_unique<FEMonomialVec<2,RealType>>(fet);
      ...
      case 3:
      {
        ...
        {
          case LAGRANGE_VEC:
            return libmesh_make_unique<FELagrangeVec<3,RealType>>(fet);

          case MONOMIAL_VEC:
            return libmesh_make_unique<FEMonomialVec<3,RealType>>(fet);
...
template <typename RealType>
struct InfBuilder<Real, RealType>
{
  static std::unique_ptr<FEGenericBase<Real, RealType>> build(const unsigned int dim,
                                                              const FEType & fet)
  {
    switch (dim)
    {
...
template <typename RealType>
struct InfBuilder<RealVectorValue, RealType>
{
  static std::unique_ptr<FEGenericBase<RealVectorValue>> build(const unsigned int,
                                                               const FEType & )
  {
    // No vector types defined... YET.
    libmesh_not_implemented();
    return std::unique_ptr<FEVectorBase<RealType>>();
  }
};
...
template <typename OutputType, typename RealType>
inline
FEGenericBase<OutputType,RealType>::FEGenericBase(const unsigned int d,
                                                  const FEType & fet) :
  FEAbstract<RealType>(d,fet),
...
template <typename OutputType, typename RealType>
inline
FEGenericBase<OutputType,RealType>::~FEGenericBase()
{
}
...
template <typename OutputType, typename RealType>
std::unique_ptr<FEGenericBase<OutputType, RealType>>
FEGenericBase<OutputType,RealType>::build (const unsigned int dim,
                                           const FEType & fet)
{
  return Builder<OutputType,RealType>::build(dim, fet);
}
...
template <typename OutputType, typename RealType>
std::unique_ptr<FEGenericBase<OutputType, RealType>>
FEGenericBase<OutputType, RealType>::build_InfFE (const unsigned int dim,
                                                  const FEType & fet)
{
  return InfBuilder<OutputType,RealType>::build(dim, fet);
}
...
template <typename OutputType, typename RealType>
void FEGenericBase<OutputType, RealType>::compute_shape_functions (const Elem * elem,
                                                                   const std::vector<Point> & qp)
{
  ...
  if (this->calculate_curl_phi && TypesEqual<OutputType,RealGradient>::value)
    this->_fe_trans->map_curl(this->dim, elem, qp, (*this), this->curl_phi);

  // Only compute div for vector-valued elements
  if (this->calculate_div_phi && TypesEqual<OutputType,RealGradient>::value)
    this->_fe_trans->map_div(this->dim, elem, qp, (*this), this->div_phi);
}


template <typename OutputType, typename RealType>
void FEGenericBase<OutputType, RealType>::print_phi(std::ostream & os) const
{
...
template <typename OutputType, typename RealType>
void FEGenericBase<OutputType, RealType>::print_dphi(std::ostream & os) const
{
...
template <typename OutputType, typename RealType>
void FEGenericBase<OutputType, RealType>::determine_calculations()
{
  ...
  if (!this->calculate_phi && !this->calculate_dphi &&
      ...
      !this->calculate_div_phi && !this->calculate_map)
    {
      ...
      if (FEInterface::field_type(this->fe_type.family) == TYPE_VECTOR)
        {
...
template <typename OutputType, typename RealType>
void FEGenericBase<OutputType, RealType>::print_d2phi(std::ostream & os) const
{
  for (auto i : index_range(dphi))
    for (auto j : index_range(dphi[i]))
...
template <typename OutputType, typename RealType>
void
FEGenericBase<OutputType, RealType>::coarsened_dof_values(const NumericVector<Number> & old_vector,
...
template <typename OutputType, typename RealType>
void
FEGenericBase<OutputType, RealType>::compute_proj_constraints (DofConstraints & constraints,
                                                               DofMap & dof_map,
...
template <typename OutputType, typename RealType>
void
FEGenericBase<OutputType, RealType>::
