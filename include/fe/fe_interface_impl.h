                               ...
                               FEComputeData & data)
{
  ...
  const PointTempl<RealType> & p = data.p;
  ...
  if (data.need_derivative())
    {
      ...
      std::vector<PointTempl<RealType>> pt(1);
      pt[0]=p;
                                                ...
                                                const ElemTempl<RealType> * elem)
{
  ...
  FEGenericBase<Real,RealType>::compute_periodic_constraints (constraints,
                                                              dof_map,
                                                              boundaries,
                                                              mesh,
                                                              point_locator,
...
#define INSTANTIATE_FE_INTERFACE_METHODS0(RealType)                                                \
  ...
  template FEInterface::shape_deriv_ptr<RealType> FEInterface::shape_deriv_function<RealType>(     \
      const unsigned int dim, const FEType & fe_t);                                                \
  template void FEInterface::compute_data(const unsigned int dim,                                  \
                                          const FEType & fe_t,                                     \
                                          const ElemTempl<RealType> * elem,                        \
                                          FEComputeData & data);                                   \
...
#ifdef LIBMESH_ENABLE_AMR
#define INSTANTIATE_FE_INTERFACE_METHODS2(RealType)                                                \
  INSTANTIATE_FE_INTERFACE_METHODS1(RealType);                                                     \
  template void FEInterface::compute_constraints(DofConstraints & constraints,                     \
                                                 DofMap & dof_map,                                 \
                                                 const unsigned int variable_number,               \
                                                 const ElemTempl<RealType> * elem)
#else
#define INSTANTIATE_FE_INTERFACE_METHODS2(RealType) INSTANTIATE_FE_INTERFACE_METHODS1(RealType)
#endif

#ifdef LIBMESH_ENABLE_PERIODIC
#define INSTANTIATE_FE_INTERFACE_METHODS3(RealType)                                                \
  INSTANTIATE_FE_INTERFACE_METHODS2(RealType);                                                     \
  template void FEInterface::compute_periodic_constraints(DofConstraints & constraints,            \
                                                          DofMap & dof_map,                        \
                                                          const PeriodicBoundaries & boundaries,   \
                                                          const MeshBaseTempl<RealType> & mesh,    \
                                                          const PointLocatorBase * point_locator,  \
                                                          ...
                                                          const ElemTempl<RealType> * elem)
#else
#define INSTANTIATE_FE_INTERFACE_METHODS3(RealType) INSTANTIATE_FE_INTERFACE_METHODS2(RealType)
#endif

#ifdef LIBMESH_ENABLE_INFINITE_ELEMENTS
#define INSTANTIATE_FE_INTERFACE_METHODS(RealType)                                                 \
  INSTANTIATE_FE_INTERFACE_METHODS3(RealType);                                                     \
...
#else
#define INSTANTIATE_FE_INTERFACE_METHODS(RealType) INSTANTIATE_FE_INTERFACE_METHODS3(RealType)
#endif
